	...
	private double getSelectedPrimaryAxisValue(int position, String orientation) {
		...
		if(baseChart != null) {
			...
			if(orientation.equals(IExtendedChart.X_AXIS)) {
				start = baseChart.getMinX();
				stop = baseChart.getMaxX();
				length = baseChart.getClientArea().width;
			} else {
				start = baseChart.getMinY();
				stop = baseChart.getMaxY();
				length = baseChart.getClientArea().height;
			}
			//
			if(position <= 0) {
				primaryValue = start;
