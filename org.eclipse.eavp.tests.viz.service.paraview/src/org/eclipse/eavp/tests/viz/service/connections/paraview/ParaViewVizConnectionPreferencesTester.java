...
/**
 * A class to test the ParaViewVizConnectionPreferences for the ability to
 * correctly manage a set of preferences through the IRemoteConnectionManager.
 * 
 * @author Robert Smith
 *
 */
public class ParaViewVizConnectionPreferencesTester {
	...
	public void checkConstruction() {
		ParaViewVizConnectionPreferences prefs = new ParaViewVizConnectionPreferences();
		...
		assertEquals(1, prefs.getVisualizerPort());
	}
	...
	public void checkSerialize() {
		...
		assertEquals("path/to/paraview.exe", prefs.getExecutablePath());
		assertEquals("/path/to/script.py", prefs.getScriptPath());
		assertEquals(9876, prefs.getVisualizerPort());
		...
		assertEquals(
				"ADDRESS_ATTR,Test Connection,1234,USERNAME_ATTR,Remote Connection,path/to/paraview.exe,/path/to/script.py,9876",
				serialized);
	...
	/**
	 * An extension of PasraViewConnectionPreferences which allows a custom
